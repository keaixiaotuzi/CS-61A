...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        print('debug:', self.__class__.__name__, self.is_doubled)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # print('debug:', 'running', self.place.__dict__)
        logic_location = self.place
        counter = 0
        while logic_location.is_hive == False:
            # print('debug:', logic_location.__dict__)
            if logic_location.bees and self.lower_bound <= counter <= self.upper_bound:
                return random_bee(logic_location.bees)  # REPLACE THIS LINE

            logic_location = logic_location.entrance
            counter += 1
            # print('debug:', logic_location.entrance)
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        #     bee.health = bee.health-amount

        ant_place = self.place
        super().reduce_health(amount)
        print('debug:', amount)
        print('debug:1', ant_place)

        for bee in ant_place.bees:
            print('debug:4', bee)
            bee.reduce_health(amount)
        if self.health <= 0:
            print('debug:2', ant_place.bees)
            # range(ant_place.bees)
            for bee in ant_place.bees[:]:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew != 0:
            self.turns_to_chew = self.turns_to_chew-1
        else:
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        print('debug:2', self.ant_contained)
        # gamestate.food += 1
        for bee in self.place.bees[:]:
            bee.reduce_health(self.damage)
        if self.ant_contained:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        logic_location = self.place.exit
        while logic_location:
            print('debug:', logic_location, logic_location.ant)
            if logic_location.ant:
                logic_location.ant.double()
            # for bee in logic_location.bees:
                # bee.reduce_health(self.damage)
            logic_location = logic_location.exit
